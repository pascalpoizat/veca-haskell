...
unittests :: TestTree
unittests = testGroup "Unit tests" [uEq, uLocationKind, uAsXta]
...
uEq = testGroup
  ...
  [ testCase "equality on clocks" $ (c == c) @?= True
  , testCase "equality on clocks (not equal)" $ (c == c') @?= False
  , testCase "equality on locations" $ (l == l) @?= True
  , testCase "equality on locations (not equal)" $ (l == l') @?= False
  , testCase "equality on clock constraint" $ (cc1 == cc1) @?= True
  ...
  $   (cc1 == cc1c)
  @?= False
  , testCase "equality on edge (no guard, no resets)" $ (e == e) @?= True
  , testCase "equality on edge (guard + resets)" $ (e2 == e2) @?= True
  , testCase "equality on edge (reordering of guard)" $ (e2 == e2x) @?= True
  , testCase "equality on edge (reordering of resets)" $ (e2 == e2y) @?= True
  , testCase "equality on edge (not equal / source)" $ (e2 == e2a) @?= False
  , testCase "equality on edge (not equal / target)" $ (e2 == e2b) @?= False
  , testCase "equality on edge (not equal / action)" $ (e2 == e2c) @?= False
  , testCase "equality on edge (not equal / guard)" $ (e2 == e2d) @?= False
  , testCase "equality on edge (not equal / resets)" $ (e2 == e2e) @?= False
  ]

uLocationKind :: TestTree
uLocationKind = testGroup
  "Unit tests for location kinds"
  [ testCase "isCommitted on a committed state"
  $   isCommitted ta_model002 (head ls)
  @?= True
  , testCase "isCommitted on an urgent state"
  $   isCommitted ta_model002 (ls !! 1)
  @?= False
  , testCase "isCommitted on an normal state"
  $   isCommitted ta_model002 (ls !! 2)
  @?= False
    --
  , testCase "isUrgent on a committed state"
  $   isUrgent ta_model002 (head ls)
  @?= False
  , testCase "isUrgent on an urgent state"
  $   isUrgent ta_model002 (ls !! 1)
  @?= True
  , testCase "isUrgent on an normal state"
  $   isUrgent ta_model002 (ls !! 2)
  @?= False
    --
  , testCase "setCommited on a committed state"
  $   isCommitted (setCommitted ta_model002 (head ls)) (head ls)
  @?= True
  , testCase "setCommited on an urgent state (1)"
  $   isCommitted (setCommitted ta_model002 (ls !! 1)) (ls !! 1)
  @?= True
  , testCase "setCommited on an urgent state (2)"
  $   isUrgent (setCommitted ta_model002 (ls !! 1)) (ls !! 1)
  @?= False
  , testCase "setCommited on a normal state"
  $   isCommitted (setCommitted ta_model002 (ls !! 2)) (ls !! 2)
  @?= True
    --
  , testCase "setUrgent on an urgent state"
  $   isUrgent (setUrgent ta_model002 (ls !! 1)) (ls !! 1)
  @?= True
  , testCase "setUrgent on a committed state (1)"
  $   isUrgent (setUrgent ta_model002 (head ls)) (head ls)
  @?= True
  , testCase "setUrgent on a commited state (2)"
  $   isCommitted (setUrgent ta_model002 (head ls)) (head ls)
  @?= False
  , testCase "setUrgent on a normal state"
  $   isUrgent (setUrgent ta_model002 (ls !! 2)) (ls !! 2)
  @?= True
  ]
 where
  ls       = Location <$> [0 .. 4 :: Int]
  ...
  ta_model002 = TimedAutomaton
    ...
    empty
    [tau]
    [ Edge (head ls) tau [] (ClockReset <$> [head cs]) [] (ls !! 1)
    , Edge (ls !! 1) tau [ClockConstraint (head cs) GE 5] [] [] (ls !! 2)
    ]
    []

uAsXta :: TestTree
uAsXta = testGroup
  "Unit tests for toXta"
  [ testCase "internal actions"
  $   asXta (TimedAutomataNetwork [ta_model001])
  @?= res1
  , testCase "internal actions + single reset + single clause guard"
  $   asXta (TimedAutomataNetwork [ta_model002])
  @?= res2
  , testCase "internal actions + multiple resets + multiple clause guard"
  $   asXta (TimedAutomataNetwork [ta_model003])
  @?= res3
  , testCase "internal actions + both guard and reset on an edge"
  $   asXta (TimedAutomataNetwork [ta_model004])
  @?= res4
  , testCase "channels and synchronized actions"
  $   asXta (TimedAutomataNetwork [ta_model005])
  @?= res5
  , testCase "invariants (single occurence of state)"
  $   asXta (TimedAutomataNetwork [ta_model006])
  @?= res6
 ...
 where
  ...
  ta_model001 = TimedAutomaton
    ...
    empty
    [tau]
    [ Edge (head ls) tau [] [] [] (ls !! 1)
    , Edge (head ls) tau [] [] [] (ls !! 2)
    ]
    []
  res1 = unlines
    [ ""
  ...
  ta_model003 = TimedAutomaton
    ...
    [head cs, cs !! 1]
    (fromList
      [ (Name ["x"], VariableTyping (Name ["x"]) (IntType NoBounds) Nothing)
      , ( Name ["y"]
        , VariableTyping (Name ["y"])
                         (IntType NoBounds)
                         (Just $ Expression "0")
        )
      , ( Name ["z", "1"]
        , VariableTyping (Name ["z", "1"])
                         (IntType (Bounds 0 4))
                         (Just $ Expression "1")
        )
      ]
    ...
    , Edge
      (ls !! 1)
      tau
      [ClockConstraint (head cs) GE 5, ClockConstraint (cs !! 1) GE 3]
      []
  ...
  ta_model004 = TimedAutomaton
    ...
    , Edge (ls !! 1)
           tau
           [ClockConstraint (head cs) GE 5]
           (ClockReset <$> [head cs])
           []
           (ls !! 2)
    , Edge (ls !! 2) tau [ClockConstraint (head cs) GE 3] [] [] (ls !! 3)
    ]
    []
  res4 = unlines
    [ ""
  ...
  ta_model006 = TimedAutomaton
    ...
    empty
    [tau]
    [ Edge (head ls) tau [] (ClockReset <$> [head cs, cs !! 1]) [] (ls !! 1)
    , Edge
      (ls !! 1)
      tau
      [ClockConstraint (head cs) TA.GT 2, ClockConstraint (cs !! 1) TA.GT 4]
      []
      []
      (ls !! 2)
    ]
    [ ( ls !! 1
      , [ClockConstraint (head cs) TA.LT 10, ClockConstraint (cs !! 1) TA.LT 8]
      )
    ]
  --
  ta_model006' :: TimedAutomaton (CTIOEvent String) Int
  ta_model006' = TimedAutomaton
    ...
    [ (ls !! 1, [ClockConstraint (head cs) TA.LT 10])
    , (ls !! 1, [ClockConstraint (cs !! 1) TA.LT 8])
    ]
  res6 = unlines
    [ ""
    , "process Model006(){"
